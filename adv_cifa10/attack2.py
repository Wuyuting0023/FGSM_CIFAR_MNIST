...
class Model:
	def __init__(self):
		self.x = tf.placeholder(tf.float32, shape=(None,32,32,3)) #tf.placeholder:define (data type, shape ,name) The input shape is Minist shape
		self.labels = tf.placeholder(tf.int32, shape=(None)) #label define,
		self.conv1  = tf.layers.conv2d(inputs=self.x,filters=64,kernel_size=[3,3],padding="same",activation=tf.nn.relu)
		self.conv2  = tf.layers.conv2d(inputs=self.conv1,filters=64,kernel_size=[3,3],padding="same",activation=tf.nn.relu)
		self.pooling1 = tf.layers.max_pooling2d(inputs=self.conv2, pool_size=[2, 2], strides=2)
		self.conv3  = tf.layers.conv2d(inputs=self.pooling1,filters=128,kernel_size=[3,3],padding="same",activation=tf.nn.relu)	
		self.conv4  = tf.layers.conv2d(inputs=self.conv3,filters=128,kernel_size=[3,3],padding="same",activation=tf.nn.relu)
		self.pooling2 = tf.layers.max_pooling2d(inputs=self.conv4, pool_size=[2, 2], strides=2)	
		self.x_flat = tf.layers.Flatten()(self.pooling2) #flatten  Layer1
		self.layer1 = tf.layers.dense(self.x_flat, 256, tf.nn.relu)#fully connected with input from: self.x_flat , Size of output :128 unit , activation =Relu  , Layer2
		self.layer2 = tf.layers.dense(self.layer1, 256, tf.nn.relu)#fully connected with input from: self.x_flat , Size of output :128 unit , activation =Relu  , Layer2
		self.dropout = tf.layers.dropout(self.layer2, rate=0.20)#tf.layers.dropout(inputs,rate=0.5,noise_shape=None,seed=None,training=False,name=None ) ,  Layer3
...
# target_class
		self.cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.reshape(self.target_class_input,[-1]),logits=model.logits) #tf.reshape(self.labels,[-1])
...
#Return adv_imges and loss between adv_imges and self.label
	...
	# Save image
	if save:
		matplotlib.pyplot.imsave('adv_cifa_img/target_' + str(target_class) + '_base_' + str(label) + '_' + str(num) + '.png', adv_img)
